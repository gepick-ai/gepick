...
import { CustomAgentDescription, PromptCustomizationService } from '@gepick/ai-core/common';
import { BinaryBuffer, DisposableCollection, Emitter, Event, InjectableService, PostConstruct, URI, createServiceDecorator } from '@gepick/core/common';
import { IOpenerService } from '@gepick/core/browser';
import { IAICorePreferenceProxy, PREFERENCE_NAME_PROMPT_TEMPLATES } from './ai-core-preferences';
import { PROMPT_TEMPLATE_EXTENSION } from './prompttemplate-contribution';

// TODO(@jaylenchen): 补充相关模块后替换这里为相关缺失模块的具体定义
const IEnvVariablesServer = createServiceDecorator("EnvVariablesServer");
type IEnvVariablesServer = any;
const IFileService = createServiceDecorator("FileService");
type IFileService = any;
type FileChangesEvent = any;
...
export class FrontendPromptCustomizationServiceImpl extends InjectableService implements PromptCustomizationService {
  ...
  constructor(
        @IEnvVariablesServer protected readonly envVariablesServer: IEnvVariablesServer,
        @IAICorePreferenceProxy protected readonly preferences: IAICorePreferenceProxy,
  ...
  protected async processTemplateDirectory(
    ...
    this.toDispose.push(this.fileService.onDidFilesChange(async (event: FileChangesEvent) => {
      if (event.changes.some((change: any) => change.resource.toString().endsWith('customAgents.yml'))) {
        this.onDidChangeCustomAgentsEmitter.fire();
      }
  ...
  protected async getTemplatesDirectoryURI(): Promise<URI> {
    const templatesFolder = this.preferences.get(PREFERENCE_NAME_PROMPT_TEMPLATES);
  ...
  protected async loadCustomAgentsFromDirectory(
    ...
    try {
      ...
      console.debug(`Error loading customAgents.yml from ${directoryURI.toString()}: ${(e as Error).message}`, e);
    }
  }
  ...
  async getCustomAgentsLocations(): Promise<{ uri: URI; exists: boolean }[]> {
    ...
    for (const dirPath of this.additionalTemplateDirs) {
      ...
      locations.push({ uri: agentsUri, exists });
    }
    return locations;
  }
