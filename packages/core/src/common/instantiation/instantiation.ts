...
export function createContributionsDecorator<T = unknown>(contributionId: symbol): ServiceIdentifier<T> {
  return multiInject(contributionId) as ServiceIdentifier<T>
}
...
export abstract class InjectableService extends Disposable {
  ...
  static createContributionsDecorator<T = unknown>(contributionId: symbol): ServiceIdentifier<T> {
    return multiInject(contributionId) as ServiceIdentifier<T>
  }
...
export function Contribution(contributionId: symbol) {
