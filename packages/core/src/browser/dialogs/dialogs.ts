import { CancellationTokenSource, IDisposable, InjectableService, Key, MaybePromise, Unmanaged, createServiceDecorator, toDisposable } from '@gepick/core/common';
import { ApplicationContribution } from '../application';
import { AbstractWidget, Message, Widget, WidgetUtilities } from '../widget';

export class DialogProps extends InjectableService {
...
export const IDialogProps = createServiceDecorator<IDialogProps>(DialogProps.name);
...
export class DialogOverlayService extends InjectableService implements ApplicationContribution {
  ...
  push(dialog: AbstractDialog<any>): IDisposable {
    if (this.documents.findIndex(document => document === dialog.node.ownerDocument) < 0) {
      WidgetUtilities.addKeyListener(dialog.node.ownerDocument.body, Key.ENTER, e => this.handleEnter(e));
      WidgetUtilities.addKeyListener(dialog.node.ownerDocument.body, Key.ESCAPE, e => this.handleEscape(e));
...
export abstract class AbstractDialog<T> extends AbstractWidget {
  ...
  constructor(
    ...
    this.closeCrossNode.classList.add(...WidgetUtilities.codiconArray('close', true));
  ...
  /**
     ...
     */
  protected preventTabbingOutsideDialog(elements = Array.from(this.node.ownerDocument.body.children)): IDisposable {
    ...
    return toDisposable(() => nonInertElements.forEach(child => child.removeAttribute('inert')));
  }
  ...
  protected setErrorMessage(error: DialogError): void {
    ...
    this.errorMessageNode.textContent = DialogError.getMessage(error);
  }
...
export const IConfirmDialogProps = createServiceDecorator<IConfirmDialogProps>(ConfirmDialogProps.name);
export type IConfirmDialogProps = ConfirmDialogProps;

export class ConfirmDialog extends AbstractDialog<boolean> {
  protected confirmed = true;

  constructor(
        @IConfirmDialogProps protected override readonly props: IConfirmDialogProps,
  ) {
    super(props);
...
export const IConfirmSaveDialogProps = createServiceDecorator<IConfirmSaveDialogProps>(ConfirmSaveDialogProps.name);
export type IConfirmSaveDialogProps = ConfirmSaveDialogProps;
...
export class ConfirmSaveDialog extends AbstractDialog<boolean | undefined> {
  ...
  constructor(
        @IConfirmSaveDialogProps protected override readonly props: IConfirmSaveDialogProps,
  ) {
    super(props);
...
export const ISingleTextInputDialogProps = createServiceDecorator<ISingleTextInputDialogProps>(SingleTextInputDialogProps.name);
export type ISingleTextInputDialogProps = SingleTextInputDialogProps;

export class SingleTextInputDialog extends AbstractDialog<string> {
  protected readonly inputField: HTMLInputElement;

  constructor(
        @ISingleTextInputDialogProps protected override props: ISingleTextInputDialogProps,
  ) {
    super(props);
