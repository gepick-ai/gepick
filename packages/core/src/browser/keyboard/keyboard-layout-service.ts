import { Emitter, Event, InjectableService, Key, KeyCode, createServiceDecorator, isWindows } from "@gepick/core/common";
...
export class KeyboardLayoutService extends InjectableService {
  private currentLayout?: KeyboardLayout;

  protected readonly layoutProvider = { getNativeLayout() {} };
  protected readonly layoutChangeNotifier = {} as any;
  protected readonly keyValidator? = {} as any;

  constructor(
    // @IKeyboardLayoutProvider protected readonly layoutProvider: IKeyboardLayoutProvider,
    // @IKeyboardLayoutChangeNotifier protected readonly layoutChangeNotifier: IKeyboardLayoutChangeNotifier,
    // @Optional() @IKeyValidator protected readonly keyValidator?: IKeyValidator;
  ) {
    super();
  }

  protected updateLayout(newLayout: any): KeyboardLayout {
  ...
  async initialize(): Promise<void> {
    this.layoutChangeNotifier.onDidChangeNativeLayout((newLayout: any) => this.updateLayout(newLayout));
  ...
  protected transformNativeLayout(nativeLayout: any): KeyboardLayout {
    ...
    for (const code in mapping) {
      ...
      if (mapping.hasOwnProperty(code)) {
        ...
        if (mappedKey && this.shouldIncludeKey(code)) {
          if (isWindows) {
            this.addWindowsKeyMapping(key2KeyCode, mappedKey, (keyMapping as any).vkey, keyMapping.value);
          }
          else {
            if (keyMapping.value) {
...
export const IKeyboardLayoutService = createServiceDecorator<IKeyboardLayoutService>(KeyboardLayoutService.name);
export type IKeyboardLayoutService = KeyboardLayoutService;
