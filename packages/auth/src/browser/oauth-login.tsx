...
import { preflightOauth } from "@gepick/auth/browser"
import { OAuthProvider } from "@gepick/auth/common"
import "@gepick/auth/browser/style/oauth-login.scss"

export const OauthLoginButton = defineComponent({
  name: "OauthLogin",

  setup() {
    const authUri = ref<string>('')

    onMounted(async () => {
      const response = await preflightOauth({
        provider: OAuthProvider.Google,
        redirectUri: "/",
      })

      authUri.value = response.authUri
    })

    function handleGoogleSignIn() {
      if (authUri.value) {
        window.open(authUri.value, '_blank', `noreferrer noopener`)
      }
    }

    return () => (
      <div class="oauth-login-btn" onClick={handleGoogleSignIn}>
        <div class="oauth-icon">
          <svg viewBox="0 0 16 16" width="100%" height="100%" fill="none" style="user-select: none; flex-shrink: 0;">
        ...
        <div class="oauth-title">谷歌用户登录</div>
      </div>
    )
  },
})
