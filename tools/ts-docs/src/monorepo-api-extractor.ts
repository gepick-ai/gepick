...
const typeScriptPath = path.resolve(require.resolve('typescript/package.json'), '..');
const debug = debugFactory('gepick:ts-docs');
...
export async function runExtractorForPackage(
...
function invokeExtractorForPackage(
  ...
  try {
    return invokeExtractor(extractorConfig, options);
  }
  catch (err) {
